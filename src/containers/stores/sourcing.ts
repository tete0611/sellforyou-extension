...
export class sourcing {
    ...
    setCategoryInput = async (value: string) => {
    ...
    searchProduct = async (e: any) => {
        ...
        while (result.length < this.searchInfo.maxLimits) {
            ...
            await Promise.all(products.map(async (v: any) => {
                if (parseInt(v.item.purchaseCnt) < this.searchInfo.purchaseCnt ||
    ...
    download = async () => {
        ...
        let result = sortBy(await Promise.all(this.result.map(async (v: any) => {
            ...
            chrome.downloads.download({ url: image_url, filename: `${path_name}/${v.rank}.${image_type}`, saveAs: false });
        ...
        chrome.downloads.download({ url: workurl, filename: `${path_name}/상품소싱목록.xlsx`, saveAs: false });
    }
}