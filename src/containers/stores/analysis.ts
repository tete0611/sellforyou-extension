...
export class analysis {
    ...
    searchKeywordByNaver = async () => {
        ...
        let result_array = await Promise.all(sliced.map(async (keyword: any, index: number) => {
        ...
        for (let i in result_sorted) {
            result_sorted[i]['카테고리'] = category_info.data.searchManyCategoryInfoA077BySomeone.find((v: any) => v.code === result_sorted[i]['카테고리'])?.name ?? "-";
        }
