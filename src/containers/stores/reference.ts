...
export class reference {
    ...
    searchKeyword = async () => {
        ...
        let naverACResp = await fetch(`https://ac.shopping.naver.com/ac?frm=shopping&q=${encodeURI(this.searchInfo.keyword)}&q_enc=UTF-8&r_cr=110111&r_enc=UTF-8&r_format=json&r_lt=110111&r_unicode=0&st=110111&t_koreng=1`);
        ...
        let streetACResp = await fetch(`https://www.11st.co.kr/AutoCompleteAjaxAction.tmall?method=getAutoCompleteJson&q=${encodeURI(encodeURI(this.searchInfo.keyword))}&callback=`);
        ...
        let auctionACResp = await fetch("http://suggest.auction.co.kr/Suggest/SuggestWebService.asmx", {
            ...
            "body": `<?xml version='1.0' encoding='utf-8'?><soap:Envelope xmlns:xsi='http://www.w3.org/2001/XMLSchema-instance' xmlns:xsd='http://www.w3.org/2001/XMLSchema' xmlns:soap='http://schemas.xmlsoap.org/soap/envelope/'><soap:Body><GetKeywordSuggest xmlns='ns'><keywordHint>${encodeURI(this.searchInfo.keyword)}</keywordHint></GetKeywordSuggest></soap:Body></soap:Envelope>`,
            "method": "POST",
        });
        ...
        let wemakepRTResp = await fetch(`https://search.wemakeprice.com/api/wmpsearch/api/v3.0/wmp-search/search.json?keyword=${encodeURI(this.searchInfo.keyword)}`);
        ...
        let tmonACResp = await fetch(`https://www.tmon.co.kr/api/direct/v1/searchextra2api/api/search/autocomplete/v2/pc?keyword=${encodeURI(this.searchInfo.keyword)}`);
        let tmonACJson = await tmonACResp.json();
        let tmonACData = tmonACJson.data.suggestKeywords.map(v => v.suggestKeyword);

        let lotteonRTResp = await fetch(`https://www.lotteon.com/search/search/search.ecn?render=search&platform=pc&q=${encodeURI(this.searchInfo.keyword)}&mallId=1`);
    ...
    download = () => {
        ...
        downloadExcel(this.searchInfo.results, "키워드분석", false);
    }
}