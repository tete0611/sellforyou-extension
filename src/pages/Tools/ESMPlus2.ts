    ...
    for (let product in data.DShopInfo.prod_codes) {
      try {
        ...
        if (JSON.parse(test.data.checkESMPlus).length >= 1) {
          ...
          productStore.addRegisteredFailed(Object.assign(market_item, { error: `(${shopName.split(" ")[0]}1.0)에 업로드된 상품으로 등록불가능합니다.` }));
        ...
        if (data.DShopInfo.site_code === "A522") {
          ...
          if (esmcategoryNumber2 === undefined) {
            productStore.addConsoleText(`(${shopName}) [${market_code}] 상품 등록 실패`);
            productStore.addRegisteredFailed(Object.assign(market_item, { error: "esm2.0에서 해당 카테고리를 지원하지 않습니다." }));
            await sendCallback(commonStore, data, market_code, parseInt(product), 2, "esm2.0에서 해당 카테고리를 지원하지 않습니다.");

            continue;
          }

          esmtogmarketoractioncategory2 = auctionOptionscategoryJson.find((item) => item.카테고리코드 == esmcategoryNumber2["A옥션"]);
        } else if (data.DShopInfo.site_code === "A523") {
          ...
          esmtogmarketoractioncategory2 = gmarketOptionscategoryJson.find((item) => item.카테고리코드 == esmcategoryNumber2["G마켓"]);
        }
        ...
        if (errorEditionalPrice === true) {
          productStore.addConsoleText(`(${shopName}) [${market_code}] 상품 등록 실패`);
          productStore.addRegisteredFailed(
            Object.assign(market_item, { error: "해당 카테고리는 추가금이 불가합니다. 옵션판매가를 모두 동일하게 설정해주세요." })
          );
        ...
        // 고시정보 생성
        const sillCode = itemInfo[`sillCode${data.DShopInfo.site_code}`] ? itemInfo[`sillCode${data.DShopInfo.site_code}`] : "35";
        ...
        let test_body = {
          ...
          LoginId: data.DShopInfo.site_code === "A522" ? commonStore.user.userInfo.esmplusAuctionId : commonStore.user.userInfo.esmplusGmarketId, //esm2.0추가
          //esm2.0추가 아래부터
          GoodsNo: null,
          SiteGoodsNo: null,
          ...
          SYIStep1: {
            ...
            RegMarketType: data.DShopInfo.site_code === "A522" ? "1" : "2",
  ...
  try {
    ...
    for (let product in data.DShopInfo.prod_codes) {
      try {
          ...
          delete_test_body = {
            ...
            SYIStep1: {
              ...
              SiteCategoryCode: [
                ...
                {
                  key: "2",
                  value: data.DShopInfo.site_code === "A522" ? "" : market_item.cate_code,
                },
              ],
              //esm2.0추가
              SiteGoodsClassList: [
            ...
            SYIStep2: {
              ...
              Price: {
                InputType: "1",
                GoodsPrice: market_item.sprice.toString(),
                GoodsPriceIAC: data.DShopInfo.site_code === "A522" ? market_item.sprice.toString() : "0",
                GoodsPriceGMKT: data.DShopInfo.site_code === "A523" ? market_item.sprice.toString() : "0",
                IsSeparate: false, //esm2.0추가
                IsUserCustomSettlementGMKT: false, //esm2.0추가
                GoodsPriceSettlementGMKT: "0", //esm2.0추가
                BookPrice: "0", //esm2.0추가
        ...
        if (data.DShopInfo.site_code === "A523") {
          //Gmarket
        ...
        } else if (data.DShopInfo.site_code === "A522") {
        ...
        try {
          ...
          if (stopJson.Unknown?.ErrorList[0]?.ErrorMessage) {
            //에러처리안되있길래 에러처리함
            productStore.addConsoleText(`(${shopName}) 상품 등록해제 중단(${stopJson.Unknown?.ErrorList[0]?.ErrorMessage})`);
            notificationByEveryTime(`(${shopName}) 상품 등록해제 도중 오류가 발생하였습니다. (${stopJson.Unknown?.ErrorList[0]?.ErrorMessage})`);
          ...
          try {
            ...
            if (deleteJson.Unknown?.ErrorList[0]?.ErrorMessage) {
              //에러처리안되있길래 에러처리함
              productStore.addConsoleText(`(${shopName}) 상품 등록해제 중단(${deleteJson.Unknown?.ErrorList[0]?.ErrorMessage})`);
              notificationByEveryTime(`(${shopName}) 상품 등록해제 도중 오류가 발생하였습니다. (${deleteJson.Unknown?.ErrorList[0]?.ErrorMessage})`);
