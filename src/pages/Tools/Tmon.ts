...
import { convertWebpToJpg, extractTmonContent, getClock, getClockOffset, notificationByEveryTime, readFileDataURL, request, sendCallback, transformContent, urlEncodedObject } from "./Common";

async function convertB956Resources(content: any) {
        ...
        for (let product in data.DShopInfo.prod_codes) {
            ...
            if (!market_item.cert) {
                if (!commonStore.uploadInfo.editable) {
                    ...
                    productStore.addRegisteredFailed(Object.assign(market_item, { error: "상품 수정 기능 준비 중입니다." }));
            ...
            // 딜번호 생성
            let codeResp: any = await request("https://spc-om.tmon.co.kr/api/deal", { "method": "POST" });
            let codeJson = JSON.parse(codeResp);

            let dealNo = codeJson.data.data;

            let categoryResp: any = await request(`https://spc-om.tmon.co.kr/api/categories/${market_item.cate_code}`, { "method": "GET" });
                ...
                "kft": [
                    {
                        ...
                        "items": [
                            ...
                            {
                                "key": "2. 법에 의한 인증·허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항",
                                "value": {
                                    "contents": "상품상세설명 참조",
                                    "refUseYN": "N"
                                },
                            ...
                            {
                                "key": "5. A/S 책임자와 전화번호 또는 소비자상담 관련 전화번호",
                                "value": {
                                    "contents": "상품상세설명 참조",
                                    "refUseYN": "N"
                                },
            ...
            let imageResp: any = await request(`https://spc-om.tmon.co.kr/api/designContent/uploadFile/${dealNo}/rule/front/maxSize/2048000`, {
                method: "POST",
            ...
            let contentBody = {
                ...
                "useInfoDesc5": "",
                "useInfoDesc6": "",
                "imgCatList3ColV2": imageJson.data.dealImagesInfoList.find((v: any) => v.imageKey === 'catlist_3col_v2').imageValue,
            ...
            market_item.content1 = await convertB956Resources(market_item.content1);

            let descBody = {
                "saveType": "DN",
                "data": JSON.stringify([{ [optionJson.data.options[0].optionNo]: `${market_item.content2}${commonStore.user.userInfo.descriptionShowTitle === "Y" ? `<br /><br /><div style="text-align: center;">${market_item.name3}</div><br /><br />` : `<br /><br />`}${transformContent(market_item.content1)}${market_item.content3}` }]),
                "mainData": JSON.stringify({ "tmplNo": "1", "depth": "1", "viewPrice": "N", "summaryCols": "column1", "saveType": "DN" })
            };
