...
import { byteSlice, getClock, getClockOffset, getStoreTraceCodeV1, notificationByEveryTime, sendCallback, transformContent } from "./Common";
  ...
  try {
    ...
    for (let product in data.DShopInfo.prod_codes) {
      try {
        ...
        const sillCode = itemInfo[`sillCode${data.DShopInfo.site_code}`] ? itemInfo[`sillCode${data.DShopInfo.site_code}`] : "891045";
        const sillData = itemInfo[`sillData${data.DShopInfo.site_code}`]
          ...
          : [
              ...
              { code: "23756033", name: "법에 의한 인증·허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항", type: "input" },
        ...
        let product_body: any = {
          ...
          data: {
            Product: {
              ...
              dlvWyCd: "01", // 배송방법: 택배
              dlvSendCloseTmpltNo:
                template_json.productInformationTemplateList.templateBOList?.find((v) => v.repCloseTimeYn[0] === "Y")?.prdInfoTmpltNo[0] ??
                "재고확인 후 순차발송", // 발송마감 템플릿번호: 재고확인 후 순차발송
