...
import { byteSlice, getClock, getClockOffset, notificationByEveryTime, sendCallback, transformContent } from "./Common";
    ...
    try {
        ...
        switch (data.DShopInfo.site_code) {
            case "A112": {
                ...
                streetOutbound = commonStore.deliveryInfo.streetGlobalOutboundList.find((v: any) => `${v.addrSeq[0]}` === `${commonStore.user.userInfo.streetDefaultOutbound}`);
                streetInbound = commonStore.deliveryInfo.streetGlobalInboundList.find((v: any) => `${v.addrSeq[0]}` === `${commonStore.user.userInfo.streetDefaultInbound}`);
            ...
            case "A113": {
                ...
                streetOutbound = commonStore.deliveryInfo.streetNormalOutboundList.find((v: any) => `${v.addrSeq[0]}` === `${commonStore.user.userInfo.streetNormalOutbound}`);
                streetInbound = commonStore.deliveryInfo.streetNormalInboundList.find((v: any) => `${v.addrSeq[0]}` === `${commonStore.user.userInfo.streetNormalInbound}`);
        ...
        for (let product in data.DShopInfo.prod_codes) {
            ...
            let product_body: any = {
                ...
                "data": {
                    "Product": {
                        ...
                        "prdImage05": market_item.img5,                     // 목록 이미지(검색 결과 페이지나 카테고리 리스트 페이지에서 노출되는 이미지) (선택)
                        ...
                        "dlvWyCd": "01",                                    // 배송방법: 택배
                        "dlvSendCloseTmpltNo": template_json.productInformationTemplateList.templateBOList?.find((v) => v.repCloseTimeYn[0] === 'Y')?.prdInfoTmpltNo[0] ?? "재고확인 후 순차발송",        // 발송마감 템플릿번호: 재고확인 후 순차발송
                        ...
                        "ProductNotification": {
                            "type": "891045",                               // 상품정보제공고시: 기타재화

                            "item": [
                                {
                                    "code": "11905",                        // 제조자/수입품의 경우 수입자를 함께 표기
                                    "name": "상세설명참조"
                                },
                                {
                                    "code": "23759100",                     // 제조국 또는 원산지
                                    "name": "상세설명참조"
                                },
                                {
                                    "code": "11800",                        // 품명 및 모델명
                                ...
                                {
                                    "code": "23760413",                     // A/S 책임자와 전화번호 또는 소비자상담 관련 전화번호
                                    "name": "상세설명참조"
                                },
                                {
                                    "code": "23756033",                     // 법에 의한 인증·허가 등을 받았음을 확인할 수 있는 경우 그에 대한 사항
                                    "name": "상세설명참조"
                                }
                            ],
                        },
            ...
            if (!market_item.cert && commonStore.uploadInfo.editable) {
                ...
                if (product_json.ClientMessage.resultCode[0] === '200') {
                    productStore.addRegisteredSuccess(Object.assign(market_item, { error: product_json.ClientMessage.productNo[0] }));
                    productStore.addConsoleText(`(${shopName}) [${market_code}] 상품 수정 성공`);

                    await sendCallback(commonStore, data, market_code, parseInt(product), 1, product_json.ClientMessage.productNo[0]);
                } else {
                    productStore.addRegisteredFailed(Object.assign(market_item, { error: product_json.ClientMessage.message[0] }));
                    productStore.addConsoleText(`(${shopName}) [${market_code}] 상품 수정 실패`);

                    await sendCallback(commonStore, data, market_code, parseInt(product), 2, product_json.ClientMessage.message[0]);
                }
            } else {
                productStore.addRegisteredQueue(market_item);
                ...
                try {
                    ...
                    if (search_json["ns2:products"]["ns2:product"]) {
                        productStore.addRegisteredSuccess(Object.assign(market_item, { error: search_json["ns2:products"]["ns2:product"][0].prdNo[0] }));
                        productStore.addConsoleText(`(${shopName}) [${market_code}] 상품 등록 성공`);

                        await sendCallback(commonStore, data, market_code, parseInt(product), 1, search_json["ns2:products"]["ns2:product"][0].prdNo[0]);
                    } else {
                        productStore.addRegisteredFailed(Object.assign(market_item, { error: search_json.ClientMessage.message[0] }));
                        productStore.addConsoleText(`(${shopName}) [${market_code}] 상품 등록 실패`);

                        await sendCallback(commonStore, data, market_code, parseInt(product), 2, search_json.ClientMessage.message[0]);
                    }
                }
            }
        }
...
async function newOrderStreet(commonStore: any, shopInfo: any) {
    ...
    try {
        ...
        const orderResults = await Promise.all(apiList.map(async (v: any) => {
            const body = {
                "apikey": v,
            ...
            return orderJson["ns2:orders"]["ns2:order"].map((w: any) => {
                return {
                    ...
                    taobaoOrderNo: null,
                    productName: v.prdNm[0],
                    productOptionContents: v.slctPrdOptNm[0],
                    sellerProductManagementCode: v.sellerPrdCd[0],
                    orderNo: v.ordNo[0],
                    orderQuantity: parseInt(v.ordQty[0]),
                    orderMemberName: v.ordNm[0],
                    orderMemberTelNo: v.ordPrtblTel[0],
                    productPayAmt: parseInt(v.ordPayAmt[0]),
                    deliveryFeeAmt: parseInt(v.dlvCst[0]),
                    individualCustomUniqueCode: v.psnCscUniqNo[0],
                    receiverName: v.rcvrNm[0],
                    receiverTelNo1: v.rcvrPrtblNo[0],
                    receiverIntegratedAddress: `${v.rcvrBaseAddr[0]} ${v.rcvrDtlsAddr[0]}`,
                    receiverZipCode: v.rcvrMailNo[0],
                    productOrderMemo: v.ordDlvReqCont[0],
                    apiKey: w
                };
            });
        }));
...
async function productPreparedStreet(commonStore: any, shopInfo: any) {
    ...
    try {
        ...
        await Promise.all(apiList.map(async (w: any) => {
            const dataInfo = orderInfo.filter((data: any) => data.apiKey === w);
            await Promise.all(dataInfo.map(async (v: any) => {
                const body = {
                    "apikey": w,
