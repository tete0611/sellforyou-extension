        ...
        for (let product in data.DShopInfo.prod_codes) {
            ...
            const productBody: any = {
                "spdLst": [{
                    ...
                    "epnLst": [{
                        "pdEpnTypCd": "DSCRP",
                        "cnts": `${market_item.content2}${commonStore.user.userInfo.descriptionShowTitle === "Y" ? `<br /><br /><div style="text-align: center;">${market_item.name3}</div><br /><br />` : `<br /><br />`}${transformContent(market_item.content1)}${market_item.content3}`
                    }],
            ...
            if (!market_item.cert && commonStore.uploadInfo.editable) {
                ...
                if (productJson.returnCode !== "0000") {
                    ...
                    if (productJson.data[0].resultCode !== "0000") {
                        productStore.addRegisteredFailed(Object.assign(market_item, { error: productJson.data[0].resultMessage }));
                        productStore.addConsoleText(`(${shopName}) 상품 수정 실패`);
...
async function newOrderLotteon(commonStore: any, shopInfo: any) {
    ...
    try {
        ...
        console.log(shopName, orderJson.data.deliveryOrderList);

        return orderJson.data.deliveryOrderList.map((v: any) => {
            return {
...
async function productPreparedLotteon(commonStore: any, shopInfo: any) {
    ...
    try {
        ...
        await Promise.all(LotteonOrderList.map(async (v: any) => {
