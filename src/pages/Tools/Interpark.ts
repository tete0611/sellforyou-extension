...
import { byteSlice, getClock, notificationByEveryTime, sendCallback, transformContent } from "./Common";
...
async function deleteInterpark(productStore: any, commonStore: any, data: any) {
    ...
    try {
        for (let product in data.DShopInfo.prod_codes) {
            ...
            const deleteJson: any = await interparkApiGateway(`product/ProductAPIService.do?_method=UpdateProductAPIData&citeKey=${commonStore.user.userInfo.interparkEditCertKey}&secretKey=${commonStore.user.userInfo.interparkEditSecretKey}&dataUrl=${process.env.SELLFORYOU_MINIO_HTTP}/${xmlText}`);
        ...
        for (let product in data.DShopInfo.prod_codes) {
            ...
            if (option_count > 0) {
                ...
                if (option_count === 1) {
                    ...
                    productStore.addRegisteredFailed(Object.assign(market_item, { error: '옵션 유형이 3개 이상인 상품은 등록하실 수 없습니다.' }));
                    productStore.addConsoleText(`(${shopName}) [${market_code}] 상품 등록 실패`);

                    await sendCallback(commonStore, data, market_code, parseInt(product), 2, "옵션 유형이 3개 이상인 상품은 등록하실 수 없습니다.");
            ...
            if (!market_item.cert && commonStore.uploadInfo.editable) {
                ...
                let product_json: any = await interparkApiGateway(`product/ProductAPIService.do?_method=UpdateProductAPIData&citeKey=${commonStore.user.userInfo.interparkEditCertKey}&secretKey=${commonStore.user.userInfo.interparkEditSecretKey}&dataUrl=${process.env.SELLFORYOU_MINIO_HTTP}/${xmlText}`);

                if (product_json.result.hasOwnProperty('error')) {
                    productStore.addRegisteredFailed(Object.assign(market_item, { error: product_json.result.error[0].explanation[0] }));
                    productStore.addConsoleText(`(${shopName}) 상품 수정 실패`);

                    await sendCallback(commonStore, data, market_code, parseInt(product), 2, product_json.result.error[0].explanation[0]);
                } else {
                    productStore.addRegisteredSuccess(Object.assign(market_item, { error: product_json.result.success[0].prdNo[0] }));
                    productStore.addConsoleText(`(${shopName}) 상품 수정 성공`);

                    await sendCallback(commonStore, data, market_code, parseInt(product), 1, product_json.result.success[0].prdNo[0]);
                }
            } else {
                productStore.addRegisteredQueue(market_item);
                ...
                let product_json: any = await interparkApiGateway(`product/ProductAPIService.do?_method=InsertProductAPIData&citeKey=${commonStore.user.userInfo.interparkCertKey}&secretKey=${commonStore.user.userInfo.interparkSecretKey}&dataUrl=${process.env.SELLFORYOU_MINIO_HTTP}/${xmlText}`);
