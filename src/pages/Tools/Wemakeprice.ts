...
import { getClock, getClockOffset, notificationByEveryTime, sendCallback, transformContent } from "./Common";
    ...
    try {
        ...
        for (let product in data.DShopInfo.prod_codes) {
            ...
            let productData: any = {
                ...
                "gnoticeList[0].gnoticeNo": "38",
                "gnoticeList[0].useYn": "Y",
                "gnoticeList[0].prodGnoticeNo": "",
                "checkboxAllNoticeExp": "기본 문구 입력",
                "gnoticeList[0].prodNoticeSetDtoList[0].prodDesc": "상품상세 설명 참고",
                "gnoticeList[0].prodNoticeSetDtoList[0].noticeNo": "195",
                "gnoticeList[0].prodNoticeSetDtoList[1].prodDesc": "상품상세 설명 참고",
                "gnoticeList[0].prodNoticeSetDtoList[1].noticeNo": "197",
                "gnoticeList[0].prodNoticeSetDtoList[2].prodDesc": "상품상세 설명 참고",
                "gnoticeList[0].prodNoticeSetDtoList[2].noticeNo": "198",
                "gnoticeList[0].prodNoticeSetDtoList[3].prodDesc": "상품상세 설명 참고",
                "gnoticeList[0].prodNoticeSetDtoList[3].noticeNo": "201",
                "gnoticeList[0].prodNoticeSetDtoList[4].prodDesc": "상품상세 설명 참고",
                "gnoticeList[0].prodNoticeSetDtoList[4].noticeNo": "203",
                ...
                "localLiquorYn": "N",
                "parallelImportYn": "N",
                "prodDesc": `${market_item.content2}${commonStore.user.userInfo.descriptionShowTitle === "Y" ? `<br /><br /><div style="text-align: center;">${market_item.name3}</div><br /><br />` : `<br /><br />`}${transformContent(market_item.content1)}${market_item.content3}`
            };
...
async function newOrderWemakeprice(commonStore: any, shopInfo: any) {
    ...
    try {
        ...
        const startDate = getClockOffset(0, 0, -1, 0, 0, 0);
        const endDate = getClock();

        const orderResp = await fetch(`https://wpartner.wemakeprice.com/ship/getOrderInfoList.json?confirmShipNoStr=&schTotalFlag=&schDateType=orderDt&schStartDate=${startDate.YY}-${startDate.MM}-${startDate.DD}&schEndDate=${endDate.YY}-${endDate.MM}-${endDate.DD}&schShipStatus=D1&schShipMethod=&schDelayShipInfoYn=&schType=&schValue=&schLimitCnt=100&schPageNo=1`, {
            "headers": {
                "accept": "application/json, text/javascript, */*; q=0.01",
                "accept-language": "ko-KR,ko;q=0.9,en-US;q=0.8,en;q=0.7",
                "content-type": "application/x-www-form-urlencoded; charset=UTF-8",
