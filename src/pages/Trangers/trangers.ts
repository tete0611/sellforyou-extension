...
async function processVisionData(info: any) {
    try {
        ...
        let visionArray: any = await visionAnalyzer(visionJson.responses[0]);
...
async function getMaskImage(itemList: any) {
    ...
    let chunk: any = await new Promise((resolve, reject) => {
        try {
            fabric.Image.fromURL(layer.image.current, async (image) => {
                image.set({
                    left: 0,
                    top: 0,
                    width: image.width,
            ...
            for (let c in vision_words) {
                ...
                for (let d in vision_symbols) {
                    ...
                    if (vision_pos.length === 0) {
                        ...
                        if (vision_paragraphs[1]) {
                            width = vision_symbols[d].boundingBox.vertices[1].x - vision_paragraphs[1].boundingBox.vertices[0].x;
                            height = vision_symbols[d].boundingBox.vertices[3].y - vision_paragraphs[1].boundingBox.vertices[0].y;
                        } else {
                            width = vision_symbols[d].boundingBox.vertices[1].x - vision_paragraphs[0].boundingBox.vertices[0].x;
                            height = vision_symbols[d].boundingBox.vertices[3].y - vision_paragraphs[0].boundingBox.vertices[0].y
                        }
...
async function addToLayers(product: any) {
    switch (params.type) {
        case "1": {
            await Promise.all(product.imageThumbnail.map(async (v: any, i: number) => {
                currentImageIndex = i;
        ...
        case "2": {
            ...
            await Promise.all(productOption.map(async (v: any, i: number) => {
                currentImageIndex = i;
        ...
        case "3": {
            ...
            if (matched) {
                let desc_resp = await fetch(`${ENDPOINT_IMAGE}/sellforyou/${product.description}`)
            ...
            await Promise.all(productDescription.map(async (v, i) => {
                currentImageIndex = i;
...
async function loadImageList(product: any) {
    imageList.innerHTML = ``;
        ...
        if (element.getAttribute('key') === buttons[i].getAttribute('key')) {
            ...
            if (editorMode === mode) {
                ...
                switch (mode) {
                    ...
                    case "download": {
                        let accept = confirm("모든 이미지가 저장됩니다.\n편집이 모두 완료되었는지 확인 후 저장해주세요.");
                        buttons[i].className = "toolbar button default btnTooltip";
...
async function multiple() {
    ...
    for (let i = 0; i < filtered.length; i++) {
        ...
        floatingToast(`번역 중... (${i + 1}/${filtered.length} 완료)`, 'inform');
    }
...
function imageToolHelper(product: any) {
    ...
    floatingTextToolDragger.addEventListener('drag', async (e: any) => {
        if (e.x === 0 && e.y === 0) {
            return;
        }
    ...
    floatingShapeToolDragger.addEventListener('drag', async (e: any) => {
        if (e.x === 0 && e.y === 0) {
            return;
        }
            ...
            switch (currentType) {
                case "0": {
                    if (appData.settings.originWidthPC === "N") {
                        dataUrl = await displayImage(filtered[i].index, appData.settings.originWidthPCSize);
                    } else {
                ...
                case "1": {
                    if (appData.settings.originWidthThumbnail === "N") {
                        dataUrl = await displayImage(filtered[i].index, appData.settings.originWidthThumbnailSize);
                    } else {
                        dataUrl = await displayImage(filtered[i].index, 0);
                    }
                ...
                case "2": {
                    if (appData.settings.originWidthOption === "N") {
                        dataUrl = await displayImage(filtered[i].index, appData.settings.originWidthOptionSize);
                    } else {
                        dataUrl = await displayImage(filtered[i].index, 0);
                    }
                ...
                case "3": {
                    if (appData.settings.originWidthDescription === "N") {
                        dataUrl = await displayImage(filtered[i].index, appData.settings.originWidthDescriptionSize);
                    } else {
                        dataUrl = await displayImage(filtered[i].index, 0);
                    }
    ...
    uploadAccept.addEventListener('click', async () => {
        loading.style.display = "";
        ...
        if (upload_json.data) {
            chrome.runtime.sendMessage({ action: "trangers", source: JSON.parse(upload_json.data.updateNewProductImageBySomeone) }, function () {
                window.close();
            });
    ...
    thisImageSave.addEventListener('click', async () => {
        let accept = confirm("저장 시 이미지를 되돌릴 수 없습니다.");
        ...
        switch (currentType) {
            case "1": {
                if (appData.settings.originWidthThumbnail === "N") {
                    dataUrl = await displayImage(currentImageIndex, appData.settings.originWidthThumbnailSize);
                } else {
            ...
            case "2": {
                if (appData.settings.originWidthOption === "N") {
                    dataUrl = await displayImage(currentImageIndex, appData.settings.originWidthOptionSize);
                } else {
                    dataUrl = await displayImage(currentImageIndex, 0);
                }
            ...
            case "3": {
                if (appData.settings.originWidthDescription === "N") {
                    dataUrl = await displayImage(currentImageIndex, appData.settings.originWidthDescriptionSize);
                } else {
                    dataUrl = await displayImage(currentImageIndex, 0);
                }
        ...
        if (upload_json.data) {
            ...
            floatingToast(`현재 이미지가 PC에 저장되었습니다.`, 'success');
...
function setTypeEvents(product: any) {
...
async function main() {
    ...
    imageToolHelper(product);

    setTypeEvents(product);
    setKeyEvents();

    await addToLayers(product);
